...
abstract class GridFeatureSpec(subjectFactory: (GridSpecification) -> GridFeature) : SubjectSpek<GridFeature>({
    val gridType = newTestGridType(5, 8)

    subject { subjectFactory(newTestGridSpecification(gridType)) }

    describe("grid") {
        it("should return game grid") {
            subject.grid.type shouldEqual gridType
        }
    }
})
